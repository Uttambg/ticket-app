...
const useTickets = () => {
  ...
  // Effect to set userRole based on authRole
  useEffect(() => {
    if (authRole) {
      setUserRole(authRole);
    }
  }, [authRole]); // Only set userRole when authRole changes

  // Effect to fetch data when userRole and id are defined
  useEffect(() => {
    if (userRole && id) {
      fetchData(); // Call fetchData only if userRole and id are defined
    }
  }, [userRole, id]); // Depend on userRole and id
